...
int derivatiba = 0;
int correccion = 0;
float ideal = 0;    //valor ideal del PID (de -250 a 250)
...
//el led trece se apaga cuando acaba la primera calibracion y se vuelve a encender cuando comienza la segunda
  ...
  if (calibracion1_ok == false) {
    ...
    do {
      ...
      for (i = 0; i < 7; i++) {
        valores[i] = 1023-analogRead(sensor[i]);
      }
  ...
  if (calibracion1_ok == true && calibracion2_ok == false) {
    ...
    do {
      ...
      for (i = 0; i < 7; i++) {
        media = media + map(1023-analogRead(sensor[i]), valores_min[i], valores_max[i], 0, 250);
      }
      media = media / 7;
  ...
  if (digitalRead(10) == false && arranque == true) {
    ...
    for (i = 0; i < 7; i++) {
      valores[i] = map(1023-analogRead(sensor[i]), valores_min[i], valores_max[i], 0, 250);
    }
    media = 0;
    for (i = 0; i < 7; i++) {
    ...
    if (((valores[6]+valores[5]) > (valores[0]+valores[1]))) {     //||posicion_anterior < -5 PEDAZO ÑAPA DE LA HOSTIA QUE REVIENTA EL CODIGO AL INTENTAR MOVER EL IDEAL, CON IDEAL 0 DE PUTA MADRE
      posicion = posicion * -1;

    }
    posicion = posicion + ideal;   //DARLE UNA PENSADA A ESTO A VER SI HAY FORMA DE HACERLO SIN QUE PETE
